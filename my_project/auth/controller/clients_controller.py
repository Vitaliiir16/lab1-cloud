...
def create_blueprint(mysql):
    ...
    @blueprint.route("/<int:client_id>/services", methods=["GET"])
    def get_client_services(client_id):
        services = dao.get_services_by_client(client_id)
    ...
    def get_client_trainer(client_id):
        ...
        trainer = trainers_dao.get_trainer_by_id(client["trainer_id"])  # Тепер це словник
        if trainer is None:
    ...
    def patch_client(client_id):
        """Частково оновити інформацію про клієнта."""
        data = request.json
